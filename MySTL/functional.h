...
// examples: plus<int> myplus; int ans = myplus(1, 2);
...
// 函数对象：加法恒等元
template <class T>
T plus_identity() {return T(0);}

// 函数对象：乘法恒等元
template <class T>
T multiple_identity() {return T(1);}
...
struct hash<float> {
    size_t operator()(const float& val) {
        return val == 0.0f ? 0 : bitwise_hash((const unsigned char*)&val, sizeof(val));
    }
};
